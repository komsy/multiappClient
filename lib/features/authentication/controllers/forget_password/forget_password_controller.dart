  ...
  resetPassword() async {
    try {
      ...
      if (setting == null || setting['appKey'] == null) {
        MFullScreenLoader.stopLoading();
        MLoaders.errorSnackBar(title: 'Error', message: "App settings could not be retrieved. Please try again.");
        return;
      }
      ...
      if (firstFiveDigits != appKey.text.trim()) {
        MFullScreenLoader.stopLoading();
        MLoaders.errorSnackBar(title: 'Invalid Input',message: "App Key does not match. Please contact support.");
        return;
      }

      if (password.text.trim() != confirmPassword.text.trim()) {
        MFullScreenLoader.stopLoading();
        MLoaders.errorSnackBar(title: 'Mismatch',message: "Passwords do not match. Please try again.");
        return;
      }
      ...
      if (rowsUpdated > 0) {
        MLoaders.successSnackBar(title: 'Success', message: 'Your password has been reset successfully.');
        Get.to(() => const ResetPasswordScreen());
      } else {
        MLoaders.errorSnackBar(title: 'Error', message: 'Failed to reset your password. Please try again later.');
      }
    } catch (e) {
      MFullScreenLoader.stopLoading();
      // debugPrint('Error resetting password: $e'); // Internal logging
      MLoaders.errorSnackBar(title: 'Oh Snap!', message: "An unexpected error occurred. Please try again later.");
    } 
  }
